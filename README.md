...
===
- **Edit** – содержит сценарии для пост-обработки билда под iOS, модифицирующие `info.plist` и `UnityAppController.mm`.  
- **Plugins/iOS** – включает нативный плагин для взаимодействия с ATT через external-методы.  
- **Scripts** – содержит C#-скрипты для работы с Yandex Mobile Ads SDK и обработки статуса ATT.  

Сценарии использования:
===
1. **Автоматический режим** (`Editor/iOSPostBuild_ATT (update info.plist + auto + manual request ATT)`)  
   - Добавляет в `info.plist` данные о политике конфиденциальности.  
   - Модифицирует `UnityAppController.mm`, добавляя автоматическую отправку статуса ATT и функцию `RequestATTFromUnity` для ручного запроса.  
   - Интегрируется с `ATTFlagSetter`.  

2. **Полуавтоматический режим** (`Editor/iOSPostBuild_ATT (update info.plist + auto request ATT)`)  
   - Обновляет `info.plist` и добавляет в `UnityAppController.mm` код для автоматической отправки статуса ATT.  

3. **Ручной режим** (`Editor/iOSPostBuild_ATT (update info.plist)`)  
   - Только обновляет `info.plist`.  
...
<key>NSUserTrackingUsageDescription</key>
    <string>Ваши данные будут использоваться для персонализации рекламы.\nYour data will be used to personalize your ads.</string>
```

Support SKAdNetwork (Yadndex Mobile Ads SDK from 15.08.25 https://ads.yandex.com/helpcenter/ru/dev/unity/quick-start?tabs=defaultTabsGroup-1mw8b55s_ios):
```XML

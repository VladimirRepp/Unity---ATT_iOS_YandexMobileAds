...
public class iOSPostBuild_ATT
{
    ...
    if (@available(iOS 14, *)) {
        ...
        // 3) Авто-вызов (с проверкой флага PlayerPrefs)
        string autoCode = @"
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@""AutoATTRequest""]) {
        ...
        if (fileContent.Contains(methodSignature))
        {
            ...
            if (returnIndex != -1 && !fileContent.Contains("AutoATTRequest"))
            {
        ...
        // 4) Ручной вызов
        if (!fileContent.Contains("RequestATTFromUnity"))
        {
            string manualMethod =
             @" #ifdef 
        ...
        UnityEngine.Debug.Log("--> iOS PostBuild: ATT код добавлен (авто + ручной вызов), Info.plist обновлён, UnitySendMessage настроен");
    }
}
